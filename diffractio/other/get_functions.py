        ...
        if len(classes) > 0:
            ...
            for cl in classes:
                if len(class_functions[cl]) > 0:
                    rst_file.write(
                        f"    Class: {cl}, Number of functions: {len(class_functions[cl])}\n\n"
                    )

    total_lines = sum(
        len(open(os.path.join(directory, file)).readlines()) for file in python_files
    )

    rst_file.write("Total\n")
    rst_file.write("============================\n\n")
