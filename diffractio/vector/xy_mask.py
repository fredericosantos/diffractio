...
class Vector_mask_XY(Vector_field_XY):
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, number_types):
            ...
            raise ValueError("other thype ({}) is not correct".format(type(other)))
    ...
    @check_none("x", "y")
    def apply_circle(
        self, r0: tuple[float, float] | None = None, radius: tuple[float, float] | None = None
    ):
    ...
    def scalar_to_vector_mask(
        self, mask: Scalar_mask_XY, pol_state: None | Jones_matrix = None, is_intensity: bool = True
    ):
    ...
    def complementary_masks(
        self,
        mask: Scalar_mask_XY,
        pol_state_0: Jones_matrix,
        pol_state_1: Jones_matrix,
    ...
    def polarizer_retarder(
        self, R: float = 0 * degrees, p1: float = 1, p2: float = 1, azimuth: float = 0 * degrees
    ):
    ...
    def radial_polarizer(self, r0: tuple[float, float] = (0.0, 0.0)):
        """Radial polarizer.
    ...
    def azimuthal_polarizer(self, r0: tuple[float, float] = (0.0, 0.0)):
    ...
    def draw(self, kind: Draw_Options = "amplitudes", range_scale: str = "um", cmap_max=1.0):
        ...
        if kind in ("amplitudes", "jones_ap"):
            ...
            fig, axs = plt.subplots(
                2, 2, sharex="col", sharey="row", gridspec_kw={"hspace": 0.25, "wspace": 0.025}
            )
