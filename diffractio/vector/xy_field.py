...
class Vector_field_XY:
    ...
    def IVFFT(
        ...
        self.pupil(r0=(0.0, 0.0), radius=radius)
    ...
    def VRS(
        ...
        # estas son las components justo en la posicion pedida
        Ex = e0x.RS(
            z=z, n=n, new_field=True, kind="z", verbose=verbose, amplification=amplification
        )
        Ey = e0y.RS(
            z=z, n=n, new_field=True, kind="z", verbose=verbose, amplification=amplification
        )
        ...
        Ez = e0z.RS(
            z=z, n=n, new_field=True, kind="0", verbose=verbose, amplification=amplification
        )
    ...
    def cut_resample(
        ...
        if new_field is False:
            ...
            field = Vector_field_XY(x=x_new, y=y_new, wavelength=self.wavelength)
            field.Ex = Ex_new
            field.Ey = Ey_new
            field.Ez = Ez_new
            return field
        ...
        if draw is True:
            if kind == "intensity":
                ...
                id_fig = self.__draw_ellipses__(
                    logarithm, normalize, cut_value, num_ellipses, amplification, **kwargs
                )
    ...
    @check_none("x", "y", "Ex", "Ey", "Ez", raise_exception=bool_raise_exception)
    def __draw_phases__(
        self, color_phase: str = CONF_DRAWING["color_phase"], draw_borders: bool = False, **kwargs
    ):
    ...
    def __draw_fields__(
        ...
        **kwargs,
        ...
        amplitude1 = normalize_draw(amplitude1, logarithm, normalize, cut_value)

        amplitude2 = np.sqrt(np.abs(Ey_r) ** 2)
        amplitude2 = normalize_draw(amplitude2, logarithm, normalize, cut_value)

        amplitude3 = np.sqrt(np.abs(Ez_r) ** 2)
        amplitude3 = normalize_draw(amplitude3, logarithm, normalize, cut_value)

        amplitude_max = np.max((amplitude1.max(), amplitude2.max(), amplitude3.max()))
        ...
        if amplitude3.max() < percentage_Ez * amplitude_max:
            ...
            plt.subplots_adjust(left=0, bottom=0, right=1, top=1, wspace=0.025, hspace=0)
            plt.tight_layout()
    ...
    def __draw1__(
        ...
        h = plt.imshow(
            image, interpolation="bilinear", aspect="auto", origin="lower", extent=extension
        )
        h.set_cmap(colormap)
        plt.axis("scaled")
        plt.axis(extension)
...
def _compute1Elipse__(
    x0: float, y0: float, A: float, B: float, theta: float, h: float = 0, amplification: float = 1
):
