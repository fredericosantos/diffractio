...
from diffractio.core.drawing import draw_several_fields
...
def gauss_beam_test():
    ...
    field.gauss_beam(
        w0=(50 * um, 50 * um), r0=(0 * um, 0 * um), z0=0, theta=0 * degrees, phi=0 * degrees
    )
...
def gauss_beam_narrow_test():
    ...
    field = Scalar_source_XY(x0, y0, wavelength)
    field.gauss_beam(
        w0=(5 * um, 5 * um), r0=(0 * um, 0 * um), theta=0 * degrees, z0=0, phi=0 * degrees
    )
...
class Test_Scalar_fields_XY:
    ...
    def test_fft(self):
        func_name = sys._getframe().f_code.co_name
        filename = f"{newpath}{func_name}.npz"

        field = field_gauss_narrow

        fieldfft1 = field.fft(z=0.1 * mm, remove0=False, shift=True, matrix=False, new_field=True)
    ...
    def test_RS(self):
        ...
        t1.square(r0=(0 * um, 0 * um), size=(40 * um, 40 * um), angle=0 * degrees)
    ...
    def test_RS_amplification(self):
        ...
        u1.gauss_beam(r0=(0.0, 0.0), w0=100 * um, z0=0, A=1, theta=0, phi=0)
    ...
    def test_profile_manual(self):
        ...
        field.draw_profile(point1=point1, point2=point2, kind="intensity", order=1)
    ...
    def test_MTF(self):
        ...
        u.MTF(fx=np.linspace(-250, 250, 512), fy=np.linspace(-250, 250, 512), has_draw=True)
    ...
    def test_send_image_screen(self):
        ...
        x0 = np.linspace(
            -pixel_size_x * num_pixels_x / 2, pixel_size_x * num_pixels_x / 2, num_pixels_x
        )
        y0 = np.linspace(
            -pixel_size_y * num_pixels_y / 2, pixel_size_y * num_pixels_y / 2, num_pixels_y
        )
        wavelength = 0.6238 * um
    ...
    def test_binarizeAmplitud(self):
        ...
        field.binarize(
            kind="amplitude", bin_level=0.5, level0=0, level1=1, new_field=False, matrix=False
        )
        field.draw(kind="intensity", normalize=None)
    ...
    def test_binarizeFase(self):
        ...
        field2 = field.binarize(
            kind="phase", bin_level=None, level0=None, level1=None, new_field=True, matrix=False
        )
    ...
    def test_discretize_1(self):
        ...
        fieldAmplitud = field.discretize(
            kind="amplitude", num_levels=2, factor=1, new_field=True, matrix=False
        )
        fieldAmplitud.draw(kind="field")

        fieldAmplitud = field.discretize(
            kind="amplitude", num_levels=2, factor=0.5, new_field=True, matrix=False
        )
        fieldAmplitud.draw(kind="field")

        fieldAmplitud = field.discretize(
            kind="amplitude", num_levels=3, factor=1, new_field=True, matrix=False
        )
        fieldAmplitud.draw(kind="field")

        fieldAmplitud = field.discretize(
            kind="amplitude", num_levels=4, factor=1, new_field=True, matrix=False
        )
        fieldAmplitud.draw(kind="field")

        fieldAmplitud = field.discretize(
            kind="amplitude", num_levels=4, factor=0.5, new_field=True, matrix=False
        )
        fieldAmplitud.draw(kind="field")
    ...
    def test_discretize_2(self):
        ...
        fieldFase = field.discretize(
            kind="phase", num_levels=2, factor=0.5, new_field=True, matrix=False
        )
        fieldFase.draw(kind="field")
        ...
        fieldFase = field.discretize(
            kind="phase", num_levels=4, factor=0.5, new_field=True, matrix=False
        )
        fieldFase.draw(kind="field")
    ...
    def test_draw_several_fields(self):
        ...
        field2.gauss_beam(
            w0=(50 * um, 100 * um), r0=(0 * um, 0 * um), z0=0, theta=0 * degrees, phi=0 * degrees
        )

        field3 = Scalar_source_XY(x=x0, y=y0, wavelength=wavelength)
        field3.laguerre_beam(A=1, r0=(0 * um, 0 * um), w0=10 * um, z=50 * um, z0=0, n=1, l=1)

        draw_several_fields(fields=(field1, field2, field3), titles=("(a)", "(b)", "(c)"))
