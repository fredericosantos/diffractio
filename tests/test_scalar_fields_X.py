...
from diffractio import eps, mm, no_date, plt, um
...
class Test_Scalar_fields_X:
    ...
    def test_cut_resample(self):
        ...
        t2 = t1.cut_resample(
            x_limits=(-250 * um, 250 * um), num_points=1024, new_field=True, interp_kind="quadratic"
        )
        t2.draw()
        save_figure_test(newpath, func_name, add_name="_2")
    ...
    def test_ifft(self):
        ...
        t3 = t2.ifft(z=None, shift=True, remove0=False, matrix=False, new_field=True, verbose=False)
        t3.draw()
    ...
    def test_MTF(self):
        ...
        u0.MTF(frequencies=np.linspace(-250, 250, 512), has_draw=True)
        plt.xlim(-250, 250)
