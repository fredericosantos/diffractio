...
def __experiment_extended_source__(x0):
    ...
    u1.spherical_wave(A=1.0, x0=x0, z0=z0, normalize=True)
    u2 = u1 * red * lens
...
class Test_Scalar_fields_X:
    ...
    def test_polychromatic(self):
        ...
        wavelengths = np.linspace(0.3 * um, 0.9 * um, 101)
        w_central = wavelengths.mean()
        spectrum_gauss = gauss_spectrum(
            wavelengths=wavelengths, w_central=w_central, Dw=0.4, normalize=True
        )

        intensity, u_s, _time_proc = polychromatic_multiprocessing(
            __function_polychromatic__, wavelengths, spectrum_gauss, num_processors=1, verbose=True
        )
        intensity0, u_s0, _time_proc0 = polychromatic_multiprocessing(
    ...
    def test_extended_source_multiprocessing(self):
        ...
        intensity, u_s, _time_proc = extended_source_multiprocessing(
            __experiment_extended_source__, x0s, num_processors=1, verbose=True
        )
        intensity0, u_s0, _time_proc0 = extended_source_multiprocessing(
    ...
    def test_grating_movement(self):
        ...
        def creation_dictionary(deltas_x, period, t1, t2):
            ...
            dict_Parameters = []
            for _i, delta_x in enumerate(deltas_x):
                dict_Parameters.append({"delta_x": delta_x, "period": period, "t1": t1, "t2": t2})
        ...
        u_s, _time_proc = execute_multiprocessing(
            __experiment_grating_movement__, dict_Parameters, num_processors, verbose=True
        )
        ...
        for i in range(0, len(deltas_x), incr_frames):
            ...
            plt.suptitle(rf"$\delta x={deltas_x[i]:6.2f}\,\mu m$", fontsize=18)
...
class Test_Scalar_fields_X_multiprocessing:
    def test_multiprocessing_dictionary(self):
        ...
        def creation_dictionary(wavelengths, x0, z, slit_size, separation):
            # create Args: for multiprocessing
            dict_Parameters = []
            for _i, wavelength in enumerate(wavelengths):
        ...
        dict_Parameters = creation_dictionary(
            wavelengths=wavelengths, x0=x0, z=0.05 * mm, slit_size=50 * um, separation=75 * um
        )

        I0 = __experiment_double_slit_dictionary__(dict_Parameters0)

        I_wavelengths, time_proc = execute_multiprocessing(
            __experiment_double_slit_dictionary__, dict_Parameters, num_processors, verbose=False
        )
    ...
    def test_multiprocessing_array(self):
        ...
        u_s, _time_proc = execute_multiprocessing(
            __experiment_double_slit_array__, slit_sizes, num_processors, verbose=True
        )

        plt.figure()
        ax = plt.subplot(111)
        for i, slit_size in enumerate(slit_sizes):
            ax.plot(u_s[i].x, abs(u_s[i].u) ** 2 + 2 * i, lw=2, label=rf"${slit_size:2.2f}\,\mu m$")
        box = ax.get_position()
    ...
    def test_extended_polychromatic_source(self):
        ...
        wavelengths = np.linspace(0.45 * um, 0.65 * um, 11)
        w_central = wavelengths.mean()
        spectrum_gauss = gauss_spectrum(
            wavelengths=wavelengths, w_central=w_central, Dw=0.4, normalize=True
        )

        u0 = __experiment_extended_polychromatic_source__({"x0": 0, "wavelength": w_central})

        intensity, u_s, _time_proc = extended_polychromatic_source(
