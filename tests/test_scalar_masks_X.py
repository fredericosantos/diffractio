...
class Test_Scalar_masks_X:
    ...
    def test_fresnel_lens(self):
        ...
        t1 = Scalar_mask_X(x, wavelength)
        t1.fresnel_lens(x0=0 * um, radius=100 * um, focal=0.25 * mm, kind="amplitude", phase=np.pi)
        t1.draw(kind="amplitude")
    ...
    def test_redBinaria_amplitude(self):
        ...
        red = Scalar_mask_X(x, wavelength)
        red.binary_grating(
            period=50 * um, a_min=0.25, a_max=0.5, phase=np.pi, x0=25 * um, fill_factor=0.25
        )
        red.draw(kind="amplitude")
    ...
    def test_redBinaria_phase(self):
        ...
        period = 40 * um
        red = Scalar_mask_X(x, wavelength)
        red.binary_grating(period=period, a_min=1, a_max=1, phase=np.pi / 2, x0=0, fill_factor=0.5)
        red.draw(kind="phase")
    ...
    def test_code(self):
        ...
        t1 = Scalar_mask_X(x, wavelength)
        t1.binary_code(kind="normal", code=code, bit_width=anchura_bit, x0=0 * um)
        t1.draw()
